...
void locationTimeStats(TelemetryData* telemetryArray, int arraySize) {
    ...
    struct sensorStats windspeedStats =
        getLocationTimeStats(telemetryArray, arraySize, "WindSpeed", locationName, startTimeEpoch, endTimeEpoch);
    struct sensorStats pressureStats =
        getLocationTimeStats(telemetryArray, arraySize, "Pressure", locationName, startTimeEpoch, endTimeEpoch);
    struct sensorStats temperatureStats =
        getLocationTimeStats(telemetryArray, arraySize, "Temperature", locationName, startTimeEpoch, endTimeEpoch);
    struct sensorStats visibilityStats =
        getLocationTimeStats(telemetryArray, arraySize, "Visibility", locationName, startTimeEpoch, endTimeEpoch);
    struct sensorStats uvRadiationStats =
        getLocationTimeStats(telemetryArray, arraySize, "UVRadiation", locationName, startTimeEpoch, endTimeEpoch);

    system("cls");
    printf("\n==== Statistics for %s between %s & %s ====", locationName, startTime, endTime);
    printf("\n\tWindspeed (km/h) Pressure (hPa)\tTemperature (C)\tVisibility "
        "(m)\tUV Index\n");
    // printf("Total:\t%.1f\t\t %.1f\t%.1f\t\t%.1f\t\t%.1f\n", windspeedStats.sum,
    //        pressureStats.sum, temperatureStats.sum, visibilityStats.sum,
...
struct sensorStats getLocationTimeStats(TelemetryData* telemetryArray,
    int arraySize, char* sensorType,
    char* locationName, time_t startTimeEpoch, time_t endTimeEpoch) {
    struct sensorStats stats;
    double stdev = 0;
    stats.sum = 0;
    stats.count = 0;
